...
AmrTracerParticleContainer* Logi::TracerPC =  0;
...
Logi::init_particles ()
{
    BL_PROFILE("Logi::init_particles()");
...
void
Logi::ParticleCheckPoint(const std::string& dir)
{
    if (level == 0)
    {
        if (TracerPC)
...
void
Logi::ParticlePlotFile(const std::string& dir)
{
    if (level == 0)
    {
...
void
Logi::ParticlePostRestart (const std::string& restart_file)
{
    if (level == 0)
    {
        if (do_tracer_particles)
...
Logi::ParticleDerive(const std::string& name,
                       Real               time,
                       int                ngrow)
{
    BL_PROFILE("Logi::ParticleDerive()");
...
void
Logi::TimestampParticles (int ngrow)
{
...
void
Logi::advance_particles(int iteration, Real time, Real dt)
{
    if (TracerPC)
    {
        int ng = iteration;
