...
bool
Furnace::react_state(MultiFab& s, MultiFab& r, Real time, Real dt, const int strang_half)
{
    BL_PROFILE("Furnace::react_state()");
...
#pragma omp parallel
    ...
    for (MFIter mfi(s, TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        [=] AMREX_GPU_HOST_DEVICE (int i, int j, int k) -> ReduceTuple
        {
            ...
            if (burn_state.T < furnace::react_T_min || burn_state.T > furnace::react_T_max ||
                burn_state.rho < furnace::react_rho_min || burn_state.rho > furnace::react_rho_max) {
                do_burn = false;
            }
    ...
    if (verbose > 0)
    {
        ...
        amrex::Print() << "Furnace::react_state() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
bool
Furnace::react_state(Real time, Real dt)
{
    ...
    for (MFIter mfi(S_new, TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        [=] AMREX_GPU_HOST_DEVICE (int i, int j, int k) -> ReduceTuple
        {
            ...
            if (U_old(i,j,k,UTEMP) < furnace::react_T_min || U_old(i,j,k,UTEMP) > furnace::react_T_max ||
                U_old(i,j,k,URHO) < furnace::react_rho_min || U_old(i,j,k,URHO) > furnace::react_rho_max) {
                do_burn = false;
            }
    ...
    if (verbose) {
        ...
        amrex::Print() << "Furnace::react_state() time = " << run_time << std::endl << std::endl;
#ifdef BL_LAZY
        });
#endif
...
bool
Furnace::valid_zones_to_burn(MultiFab& State)
{
