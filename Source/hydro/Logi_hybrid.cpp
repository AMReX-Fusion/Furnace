...
void
Logi::construct_old_hybrid_source(MultiFab& source, MultiFab& state, Real time, Real dt)
{
    BL_PROFILE("Logi::construct_old_hybrid_source()");
    ...
    if (verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Logi::construct_old_hybrid_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
void
Logi::construct_new_hybrid_source(MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt)
{
    BL_PROFILE("Logi::construct_new_hybrid_source()");
    ...
    if (verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Logi::construct_new_hybrid_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
void
Logi::fill_hybrid_hydro_source(MultiFab& sources, MultiFab& state, Real mult_factor)
{
    BL_PROFILE("Logi::fill_hybrid_hydro_source()");
...
void
Logi::linear_to_hybrid_momentum(MultiFab& state, int ng)
{
    BL_PROFILE("Logi::linear_to_hybrid_momentum()");
...
void
Logi::hybrid_to_linear_momentum(MultiFab& state, int ng)
{
    BL_PROFILE("Logi::hybrid_to_linear_momentum()");
