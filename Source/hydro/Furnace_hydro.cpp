...
#ifdef TRUE_SDC
void
Furnace::cons_to_prim(const Real time)
{

    BL_PROFILE("Furnace::cons_to_prim()");
...
void
Furnace::cons_to_prim(MultiFab& u, MultiFab& q_in, MultiFab& qaux_in, Real time)
{
...
#ifdef TRUE_SDC
void
Furnace::cons_to_prim_fourth(const Real time)
{

    BL_PROFILE("Furnace::cons_to_prim_fourth()");
...
void
Furnace::check_for_cfl_violation(const MultiFab& State, const Real dt)
{

    BL_PROFILE("Furnace::check_for_cfl_violation()");
...
#pragma omp parallel
    ...
    for (MFIter mfi(State, hydro_tile_size); mfi.isValid(); ++mfi) {
        ...
        [=] AMREX_GPU_HOST_DEVICE (int i, int j, int k) -> ReduceTuple
        {
            ...
            if (furnace::time_integration_method == 0) {
