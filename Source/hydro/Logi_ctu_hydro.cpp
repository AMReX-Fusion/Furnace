...
void
Logi::construct_ctu_hydro_source(Real time, Real dt)
{

#ifndef TRUE_SDC

  BL_PROFILE("Logi::construct_ctu_hydro_source()");
  ...
  if (!Radiation::rad_hydro_combined) {
    amrex::Abort("Logi::construct_ctu_hydro_source -- we don't implement a mode where we have radiation, but it is not coupled to hydro");
  }
  ...
  if (verbose > 0)
    {
      ...
      Lazy::QueueReduction( [=] () mutable {
        ...
        if (ParallelDescriptor::IOProcessor())
          std::cout << "Logi::construct_ctu_hydro_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
