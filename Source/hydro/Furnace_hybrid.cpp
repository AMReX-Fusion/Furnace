...
void
Furnace::construct_old_hybrid_source(MultiFab& source, MultiFab& state, Real time, Real dt)
{
    BL_PROFILE("Furnace::construct_old_hybrid_source()");
    ...
    if (verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Furnace::construct_old_hybrid_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
void
Furnace::construct_new_hybrid_source(MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt)
{
    BL_PROFILE("Furnace::construct_new_hybrid_source()");
    ...
    if (verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Furnace::construct_new_hybrid_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
void
Furnace::fill_hybrid_hydro_source(MultiFab& sources, MultiFab& state, Real mult_factor)
{
    BL_PROFILE("Furnace::fill_hybrid_hydro_source()");
...
void
Furnace::linear_to_hybrid_momentum(MultiFab& state, int ng)
{
    BL_PROFILE("Furnace::linear_to_hybrid_momentum()");
...
void
Furnace::hybrid_to_linear_momentum(MultiFab& state, int ng)
{
    BL_PROFILE("Furnace::hybrid_to_linear_momentum()");
