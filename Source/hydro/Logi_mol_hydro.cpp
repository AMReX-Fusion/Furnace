...
///
void
Logi::construct_mol_hydro_source(Real time, Real dt, MultiFab& A_update)
{
  ...
  BL_PROFILE("Logi::construct_mol_hydro_source()");
  ...
  BL_PROFILE_VAR("Logi::advance_hydro_ca_umdrv()", CA_UMDRV);
  ...
  if (print_update_diagnostics) {
    ...
    if (verbose > 0)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
          std::cout << "Logi::construct_mol_hydro_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
