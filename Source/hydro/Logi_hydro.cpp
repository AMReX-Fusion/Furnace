...
#ifdef TRUE_SDC
void
Logi::cons_to_prim(const Real time)
{

    BL_PROFILE("Logi::cons_to_prim()");
...
void
Logi::cons_to_prim(MultiFab& u, MultiFab& q_in, MultiFab& qaux_in, Real time)
{
...
#ifdef TRUE_SDC
void
Logi::cons_to_prim_fourth(const Real time)
{

    BL_PROFILE("Logi::cons_to_prim_fourth()");
...
void
Logi::check_for_cfl_violation(const MultiFab& State, const Real dt)
{

    BL_PROFILE("Logi::check_for_cfl_violation()");
...
#pragma omp parallel
    ...
    for (MFIter mfi(State, hydro_tile_size); mfi.isValid(); ++mfi) {
        ...
        [=] AMREX_GPU_HOST_DEVICE (int i, int j, int k) -> ReduceTuple
        {
            ...
            if (logi::time_integration_method == 0) {
