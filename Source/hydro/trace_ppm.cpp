...
Logi::trace_ppm(const Box& bx,
  ...
  [=] AMREX_GPU_HOST_DEVICE (int i, int j, int k)
  {
    ...
    if ((idir == 0 && i >= vlo[0]) ||
      ...
      qp(i,j,k,QREINT) = amrex::max(logi::small_dens * logi::small_ener,
    ...
    if ((idir == 0 && i <= vhi[0]) ||
      ...
      if (idir == 0) {
        ...
        qm(i+1,j,k,QREINT) = amrex::max(logi::small_dens * logi::small_ener,
                                        rhoe_g_ref + (alphap + alpham)*h_g_ref + alpha0e_g);
        ...
        qm(i,j+1,k,QREINT) = amrex::max(logi::small_dens * logi::small_ener,
                                        rhoe_g_ref + (alphap + alpham)*h_g_ref + alpha0e_g);
        ...
        qm(i,j,k+1,QREINT) = amrex::max(logi::small_dens * logi::small_ener,
                                        rhoe_g_ref + (alphap + alpham)*h_g_ref + alpha0e_g);
