...
fill_temp_cond(const Box& bx,
  ...
  [=] AMREX_GPU_DEVICE (int i, int j, int k)
  {
    ...
    if (eos_state.rho > logi::diffuse_cutoff_density) {
      conductivity(eos_state);

      if (eos_state.rho < logi::diffuse_cutoff_density_hi) {
        Real multiplier = (eos_state.rho - logi::diffuse_cutoff_density) /
          (logi::diffuse_cutoff_density_hi - logi::diffuse_cutoff_density);
        eos_state.conductivity = eos_state.conductivity * multiplier;
      }
    } else {
      eos_state.conductivity = 0.0_rt;
    }
    coeff_arr(i,j,k) = logi::diffuse_cond_scale_fac * eos_state.conductivity;
