...
void
Furnace::construct_old_diff_source(MultiFab& source, MultiFab& state_in, Real time, Real dt)
{
    BL_PROFILE("Furnace::construct_old_diff_source()");
    ...
    if (verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Furnace::construct_old_diff_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
void
Furnace::construct_new_diff_source(MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt)
{
    BL_PROFILE("Furnace::construct_new_diff_source()");
    ...
    if (verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Furnace::construct_new_diff_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
void
Furnace::add_temp_diffusion_to_source (MultiFab& ext_src, MultiFab& state_in, MultiFab& DiffTerm, Real t, Real mult_factor)
{
    BL_PROFILE("Furnace::add_temp_diffusion_to_sources()");
...
void
Furnace::getTempDiffusionTerm (Real time, MultiFab& state_in, MultiFab& TempDiffTerm)
{
    BL_PROFILE("Furnace::getTempDiffusionTerm()");
