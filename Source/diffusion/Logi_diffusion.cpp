...
void
Logi::construct_old_diff_source(MultiFab& source, MultiFab& state_in, Real time, Real dt)
{
    BL_PROFILE("Logi::construct_old_diff_source()");
    ...
    if (verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Logi::construct_old_diff_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
void
Logi::construct_new_diff_source(MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt)
{
    BL_PROFILE("Logi::construct_new_diff_source()");
    ...
    if (verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Logi::construct_new_diff_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
void
Logi::add_temp_diffusion_to_source (MultiFab& ext_src, MultiFab& state_in, MultiFab& DiffTerm, Real t, Real mult_factor)
{
    BL_PROFILE("Logi::add_temp_diffusion_to_sources()");
...
void
Logi::getTempDiffusionTerm (Real time, MultiFab& state_in, MultiFab& TempDiffTerm)
{
    BL_PROFILE("Logi::getTempDiffusionTerm()");
