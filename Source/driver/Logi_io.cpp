...
// Logi maintains an internal checkpoint version numbering system.
...
// number is stored in the LogiHeader file inside a checkpoint.
...
// 6: Simplified_SDC_Source_Type removed from Logi
...
// 9: Rotation_Type was removed from Logi
// 10: Reactions_Type was removed from checkpoints
// 11: PhiRot_Type was removed from Logi
...
// I/O routines for Logi

void
Logi::restart (Amr&     papa,
                 istream& is,
                 bool     bReadSpecial)
{
    // Let's check Logi checkpoint version first;
    // trying to read from checkpoint; if nonexisting, set it to 0.
    if (input_version == -1) {
        if (ParallelDescriptor::IOProcessor()) {
            std::ifstream LogiHeaderFile;
            std::string FullPathLogiHeaderFile = papa.theRestartFile();
            FullPathLogiHeaderFile += "/LogiHeader";
            LogiHeaderFile.open(FullPathLogiHeaderFile.c_str(), std::ios::in);
            if (LogiHeaderFile.good()) {
                char foo[256];
                // first line: Checkpoint version: ?
                LogiHeaderFile.getline(foo, 256, ':');  
                LogiHeaderFile >> input_version;
                LogiHeaderFile.close();
            } else {
                input_version = 0;
            }
        }
...
void
Logi::set_state_in_checkpoint (Vector<int>& state_in_checkpoint)
{
...
void
Logi::checkPoint(const std::string& dir,
                   std::ostream&  os,
                   VisMF::How     how,
                   bool /*dump_old_default*/)
{
  ...
  if (level == 0 && ParallelDescriptor::IOProcessor())
    {
        {
            std::ofstream LogiHeaderFile;
            std::string FullPathLogiHeaderFile = dir;
            FullPathLogiHeaderFile += "/LogiHeader";
            LogiHeaderFile.open(FullPathLogiHeaderFile.c_str(), std::ios::out);

            LogiHeaderFile << "Checkpoint version: " << current_version << std::endl;
            LogiHeaderFile.close();
...
void
Logi::writeJobInfo (const std::string& dir, const Real io_time)
{
  ...
  jobInfoFile << " Logi Job Information\n";
  jobInfoFile << PrettyLine;
  ...
  if (strlen(githash1) > 0) {
    jobInfoFile << "Logi       git describe: " << githash1 << "\n";
  }
...
Logi::writeBuildInfo ()
{
  ...
  // build information
  std::cout << PrettyLine;
  std::cout << " Logi Build Information\n";
  std::cout << PrettyLine;
  ...
  const char* githash3 = buildInfoGetGitHash(3);
  if (strlen(githash1) > 0) {
    std::cout << "Logi       git describe: " << githash1 << "\n";
  }
  if (strlen(githash2) > 0) {
...
void
Logi::writePlotFile(const std::string& dir,
                      ostream& os,
                      VisMF::How how)
{
...
void
Logi::writeSmallPlotFile (const std::string& dir,
                            ostream&       os,
                            VisMF::How     how)
{
...
void
Logi::plotFileOutput(const std::string& dir,
                       ostream& os,
                       VisMF::How how,
                       const int is_small)
{
  ...
  ParticlePlotFile(dir);
    ...
    if (store_burn_weights) {
        n_data_items += Logi::burn_weight_names.size();
    }
#endif
#endif
...
#ifndef TRUE_SDC
        if (store_burn_weights) {
            for (auto name: Logi::burn_weight_names) {
                os << name << '\n';
            }
        }
#endif
...
#ifndef TRUE_SDC
    if (store_burn_weights) {
        MultiFab::Copy(plotMF, getLevel(level).burn_weights, 0, cnt, Logi::burn_weight_names.size(),0);
        cnt += Logi::burn_weight_names.size();
    }
#endif
#endif
