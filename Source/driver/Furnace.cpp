...
bool         Furnace::signalStopJob = false;

#ifdef REACTIONS
std::vector<std::string> Furnace::burn_weight_names;
#endif

std::vector<std::string> Furnace::err_list_names;
std::vector<int> Furnace::err_list_ng;
int          Furnace::num_err_list_default = 0;
int          Furnace::radius_grow   = 1;
BCRec        Furnace::phys_bc;
int          Furnace::NUM_GROW      = -1;
int          Furnace::NUM_GROW_SRC  = -1;

int          Furnace::lastDtPlotLimited = 0;
Real         Furnace::lastDtBeforePlotLimiting = 0.0;

Real         Furnace::num_zones_advanced = 0.0;

Vector<std::string> Furnace::source_names;

Vector<AMRErrorTag> Furnace::error_tags;

Vector<std::unique_ptr<std::fstream>> Furnace::data_logs;
Vector<std::unique_ptr<std::fstream>> Furnace::problem_data_logs;

#ifdef TRUE_SDC
int          Furnace::SDC_NODES;
Vector<Real> Furnace::dt_sdc;
Vector<Real> Furnace::node_weights;
#endif

#ifdef GRAVITY
// the gravity object
Gravity*     Furnace::gravity  = 0;
#endif

#ifdef DIFFUSION
// the diffusion object
Diffusion*    Furnace::diffusion  = 0;
#endif

#ifdef RADIATION

// the radiation object
Radiation*   Furnace::radiation = 0;
#endif


std::string  Furnace::probin_file = "probin";


#if AMREX_SPACEDIM == 1
#ifndef AMREX_USE_GPU
IntVect      Furnace::hydro_tile_size(1024);
#else
IntVect      Furnace::hydro_tile_size(1048576);
#endif
IntVect      Furnace::no_tile_size(1024);
#elif AMREX_SPACEDIM == 2
#ifndef AMREX_USE_GPU
IntVect      Furnace::hydro_tile_size(1024,16);
#else
IntVect      Furnace::hydro_tile_size(1048576,1048576);
#endif
IntVect      Furnace::no_tile_size(1024,1024);
#else
#ifndef AMREX_USE_GPU
IntVect      Furnace::hydro_tile_size(1024,16,16);
#else
IntVect      Furnace::hydro_tile_size(1048576,1048576,1048576);
#endif
IntVect      Furnace::no_tile_size(1024,1024,1024);
#endif

// this will be reset upon restart
Real         Furnace::previousCPUTimeUsed = 0.0;

Real         Furnace::startCPUTime = 0.0;

int          Furnace::SDC_Source_Type = -1;
int          Furnace::num_state_type = 0;

int          Furnace::do_cxx_prob_initialize = 0;


// Furnace::variableSetUp is in Furnace_setup.cpp
...
Furnace::read_params ()
{
    ...
    if (dgeom.isAnyPeriodic())
    {
        ...
        for (int dir = 0; dir<AMREX_SPACEDIM; dir++)
        {
            if (dgeom.isPeriodic(dir))
            {
                ...
                {
                    std::cerr << "Furnace::read_params:periodic in direction "
                              << dir
    ...
    else
    {
        ...
        for (int dir=0; dir<AMREX_SPACEDIM; dir++)
        {
            if (lo_bc[dir] == Interior)
            {
                std::cerr << "Furnace::read_params:interior bc in direction "
                          << dir
                          << " but not periodic\n";
                amrex::Error();
            }
    ...
    if ( dgeom.IsRZ() && (lo_bc[0] != Symmetry) ) {
        std::cerr << "ERROR:Furnace::read_params: must set r=0 boundary condition to Symmetry for r-z\n";
        amrex::Error();
    }
    ...
    if ( dgeom.IsSPHERICAL() )
    {
      ...
      {
        std::cerr << "ERROR:Furnace::read_params: must set r=0 boundary condition to Symmetry for spherical\n";
        amrex::Error();
      }
    }
#elif (AMREX_SPACEDIM == 2)
    ...
    if (do_rotation) {
      if (rotational_period <= 0.0) {
        std::cerr << "Error:Furnace::Rotation enabled but rotation period less than zero\n";
        amrex::Error();
      }
    }
    if (dgeom.IsRZ())
...
#if (AMREX_SPACEDIM == 1)
      if (do_rotation) {
        std::cerr << "ERROR:Furnace::Rotation not implemented in 1d\n";
        amrex::Error();
      }
#endif
#endif
    ...
    prev_state(num_state_type)
{
    ...
    // Coterminous AMR boundaries are not supported in Furnace if we're doing refluxing.
    ...
    // anything that isn't known to Furnace
...
void
Furnace::setTimeLevel (Real time,
                      Real dt_old,
                      Real dt_new)
{
...
void
Furnace::init (AmrLevel &old)
{
    BL_PROFILE("Furnace::init(old)");

    Furnace* oldlev = (Furnace*) &old;
...
Furnace::estTimeStep ()
{
    BL_PROFILE("Furnace::estTimeStep()");
    ...
    std::string limiter = "furnace.max_dt";
    ...
    if (verbose) {
        amrex::Print() << "Furnace::estTimeStep (" << limiter << "-limited) at level " << level << ":  estdt = " << estdt << '\n' << std::endl;
    }
...
void
Furnace::computeNewDt (int                    finest_level,
                      int                    /*sub_cycle*/,
                      Vector<int>&           n_cycle,
...
{
    BL_PROFILE("Furnace::computeNewDt()");
              ...
              for (int i = 0; i <= finest_level; i++)
              {
                  if (verbose && ParallelDescriptor::IOProcessor()) {
                    if (dt_min[i] > change_max*dt_level[i])
                      {
                          std::cout << "Furnace::compute_new_dt : limiting dt at level "
                                    << i << '\n';
...
void
Furnace::computeInitialDt (int                   finest_level,
                          int                   /*subcycle*/,
                          Vector<int>&           n_cycle,
                          const Vector<IntVect>& /*ref_ratio*/,
                          Vector<Real>&          dt_level,
                          Real                  stop_time)
{
    BL_PROFILE("Furnace::computeInitialDt()");
...
void
Furnace::post_timestep (int iteration_local)
{
    BL_PROFILE("Furnace::post_timestep()");
...
void
Furnace::post_restart ()
{
   BL_PROFILE("Furnace::post_restart()");
...
void
Furnace::postCoarseTimeStep (Real cumtime)
{
...
void
Furnace::post_regrid (int lbase,
                     int new_finest)
{

    BL_PROFILE("Furnace::post_regrid()");
...
void
Furnace::post_init (Real /*stop_time*/)
{
    BL_PROFILE("Furnace::post_init()");
...
void
Furnace::post_grown_restart ()
{

    BL_PROFILE("Furnace::post_grown_restart()");
...
#ifdef AUX_UPDATE
void
Furnace::advance_aux(Real time, Real dt)
{
    BL_PROFILE("Furnace::advance_aux()");
...
void
Furnace::FluxRegCrseInit() {

    if (level == parent->finestLevel()) {
      return;
    }

    Furnace& fine_level = getLevel(level+1);
...
void
Furnace::FluxRegFineAdd() {

    BL_PROFILE("Furnace::FluxRegFineAdd()");
...
void
Furnace::reflux(int crse_level, int fine_level)
{
    BL_PROFILE("Furnace::reflux()");
    ...
    for (int lev = fine_level; lev > crse_level; --lev) {
        ...
        Furnace& crse_lev = getLevel(lev-1);
            ...
            for (MFIter mfi(crse_state, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
                ...
                [=] AMREX_GPU_DEVICE (int i, int j, int k)
                {
                    ...
                    if (bx.contains(i,j,k)) {
                        if (U(i,j,k,URHO) + dt * F(i,j,k,URHO) / V(i,j,k) < furnace::small_dens) {
                            zero_flux = true;
                        }
                    }
                    ...
                    if (idir == 0) {
                        if (bx.contains(i-1,j,k)) {
                            if (U(i-1,j,k,URHO) - dt * F(i,j,k,URHO) / V(i-1,j,k) < furnace::small_dens) {
                                zero_flux = true;
                            }
                        }
                    }
                        ...
                        if (bx.contains(i,j-1,k)) {
                            if (U(i,j-1,k,URHO) - dt * F(i,j,k,URHO) / V(i,j-1,k) < furnace::small_dens) {
                                zero_flux = true;
                            }
                        }
                    }
                        ...
                        if (bx.contains(i,j,k-1)) {
                            if (U(i,j,k-1,URHO) - dt * F(i,j,k,URHO) / V(i,j,k-1) < furnace::small_dens) {
                                zero_flux = true;
                            }
                        }
                    }
        ...
        if (do_grav && gravity->get_gravity_type() == "PoissonGrav" && gravity->NoSync() == 0)  {
            ...
            // 37 in the Furnace I paper. The dimensions of dphi are therefore actually
    ...
    if (verbose)
    {
        ...
        if (ParallelDescriptor::IOProcessor()) {
          std::cout << "Furnace::reflux() at level " << level << " : time = " << end << std::endl;
        }
#ifdef BL_LAZY
        });
#endif
...
void
Furnace::normalize_species (MultiFab& S_new, int ng)
{
    BL_PROFILE("Furnace::normalize_species()");
        ...
        [=] AMREX_GPU_HOST_DEVICE (int i, int j, int k)
        {
            ...
            for (int n = 0; n < NumSpec; ++n) {
                ...
                if (X < -X_failure_tolerance || X > 1.0_rt + X_failure_tolerance) {
                    ...
                    amrex::Error("Invalid mass fraction in Furnace::normalize_species()");
                }
#endif
                              ...
                              MultiFab& S)
{
    ...
    BL_PROFILE("Furnace::enforce_consistent_e()");
...
void
Furnace::enforce_min_density (MultiFab& state_in, int ng)
{

    BL_PROFILE("Furnace::enforce_min_density()");
...
void
Furnace::enforce_speed_limit (MultiFab& state_in, int ng)
{
    BL_PROFILE("Furnace::enforce_speed_limit()");
...
#pragma omp parallel
    ...
    for (MFIter mfi(state_in, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        [=] AMREX_GPU_HOST_DEVICE (int i, int j, int k)
        {
            ...
            if (v > furnace::speed_limit) {
                Real reduce_factor = furnace::speed_limit / v;
...
void
Furnace::avgDown (int state_indx)
{
    BL_PROFILE("Furnace::avgDown(state_indx)");

    if (level == parent->finestLevel()) return;

    Furnace& fine_lev = getLevel(level+1);
...
void
Furnace::removeOldData()
{
    AmrLevel::removeOldData();
}

void
Furnace::errorEst (TagBoxArray& tags,
                  int          /*clearval*/,
                  int          /*tagval*/,
                  Real         time,
                  int          /*n_error_buf*/,
...
void
Furnace::apply_problem_tags (TagBoxArray& tags, Real time)
{

    BL_PROFILE("Furnace::apply_problem_tags()");
...
void
Furnace::apply_tagging_restrictions(TagBoxArray& tags, Real time)
{
    BL_PROFILE("Furnace::apply_tagging_restrictions()");
...
Furnace::derive (const std::string& name,
                Real           time,
                int            ngrow)
{

    BL_PROFILE("Furnace::derive()");
...
void
Furnace::reset_internal_energy(const Box& bx,
#ifdef MHD
...

{
    ...
    BL_PROFILE("Furnace::reset_internal_energy()");
...
#ifdef MHD
void
Furnace::add_magnetic_e( MultiFab& Bx,
                        MultiFab& By, 
                        MultiFab& Bz,
                        MultiFab& State)
{
...
void
Furnace::check_div_B( MultiFab& Bx,
                     MultiFab& By, 
                     MultiFab& Bz,
                     MultiFab& State)
{
...

{
  ...
  BL_PROFILE("Furnace::computeTemp()");
  ...
  for (MFIter mfi(State, TilingIfNotGPU()); mfi.isValid(); ++mfi)
  {
      ...
      if (clamp_ambient_temp == 1) {
          ...
          [=] AMREX_GPU_DEVICE (int i, int j, int k)
          {
              ...
              if (u(i,j,k,URHO) <= furnace::ambient_safety_factor * ambient::ambient_state[URHO]) {
...
void
Furnace::create_source_corrector()
{

    BL_PROFILE("Furnace::create_source_corrector()");
...
void
Furnace::swap_state_time_levels(const Real dt)
{

    BL_PROFILE("Furnace::swap_state_time_levels()");
...
Furnace::get_numpts ()
{
     ...
     if (verbose && ParallelDescriptor::IOProcessor()) {
       std::cout << "Furnace::numpts_1d at level  " << level << " is " << numpts_1d << std::endl;
     }
...
void
Furnace::make_radial_data(int is_new)
{
#if (AMREX_SPACEDIM > 1)

   BL_PROFILE("Furnace::make_radial_data()");
   ...
   for (MFIter mfi(S); mfi.isValid(); ++mfi)
   {
       ...
       [=] AMREX_GPU_DEVICE(int i, int j, int k)
       {
           ...
           if (index > numpts_1d-1) {
               ...
               std::cout << "R / DR " << r << " " << dr << "\n";
               amrex::Error("Error:: Furnace_util.H :: compute_avgstate");
           }
#endif
...
void
Furnace::define_new_center(MultiFab& S, Real time)
{
    BL_PROFILE("Furnace::define_new_center()");
...
MultiFab&
Furnace::build_fine_mask()
{
    BL_PROFILE("Furnace::build_fine_mask()");
...
iMultiFab&
Furnace::build_interior_boundary_mask (int ng)
{
    BL_PROFILE("Furnace::build_interior_boundary_mask()");
...
void
Furnace::expand_state(MultiFab& S, Real time, int ng)
{
  BL_PROFILE("Furnace::expand_state()");
...
void
Furnace::check_for_nan(MultiFab& state_in, int check_ghost)
{
  BL_PROFILE("Furnace::check_for_nan()");
...
Furnace::clean_state(
    ...
    BL_PROFILE("Furnace::clean_state()");
