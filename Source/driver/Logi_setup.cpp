...
Logi::variableSetUp ()
{
  ...
  // Logi::variableSetUp is called in the constructor of Amr.cpp, so
  ...
  if (ParallelDescriptor::IOProcessor()) {
    ...
    if (strlen(logi_hash) > 0) {
      std::cout << "\n" << "Logi git describe: " << logi_hash << "\n";
    }
  ...
  eos_init(logi::small_temp, logi::small_dens);

#ifdef RADIATION
  opacity_init();
#endif

  // Ensure that Logi's small variables are consistent
  ...
  logi::small_pres = amrex::max(logi::small_pres, eos_state.p);
  logi::small_ener = amrex::max(logi::small_ener, eos_state.e);
  ...
  ambient::ambient_state[URHO]  = amrex::max(logi::ambient_density, logi::small_dens);
  ambient::ambient_state[UTEMP] = amrex::max(logi::ambient_temp, logi::small_temp);
  ambient::ambient_state[UEINT] = ambient::ambient_state[URHO] * amrex::max(logi::ambient_energy,
                                                                            logi::small_ener);
  ...
  if (UMX != URHO+1) {
    amrex::Error("We are assuming UMX = URHO + 1 in Logi_setup.cpp");
  }
