...
// Furnace maintains an internal checkpoint version numbering system.
...
// number is stored in the FurnaceHeader file inside a checkpoint.
...
// 6: Simplified_SDC_Source_Type removed from Furnace
...
// 9: Rotation_Type was removed from Furnace
// 10: Reactions_Type was removed from checkpoints
// 11: PhiRot_Type was removed from Furnace
...
// I/O routines for Furnace

void
Furnace::restart (Amr&     papa,
                 istream& is,
                 bool     bReadSpecial)
{
    // Let's check Furnace checkpoint version first;
    // trying to read from checkpoint; if nonexisting, set it to 0.
    if (input_version == -1) {
        if (ParallelDescriptor::IOProcessor()) {
            std::ifstream FurnaceHeaderFile;
            std::string FullPathFurnaceHeaderFile = papa.theRestartFile();
            FullPathFurnaceHeaderFile += "/FurnaceHeader";
            FurnaceHeaderFile.open(FullPathFurnaceHeaderFile.c_str(), std::ios::in);
            if (FurnaceHeaderFile.good()) {
                char foo[256];
                // first line: Checkpoint version: ?
                FurnaceHeaderFile.getline(foo, 256, ':');  
                FurnaceHeaderFile >> input_version;
                FurnaceHeaderFile.close();
            } else {
                input_version = 0;
            }
        }
...
void
Furnace::set_state_in_checkpoint (Vector<int>& state_in_checkpoint)
{
...
void
Furnace::checkPoint(const std::string& dir,
                   std::ostream&  os,
                   VisMF::How     how,
                   bool /*dump_old_default*/)
{
  ...
  if (level == 0 && ParallelDescriptor::IOProcessor())
    {
        {
            std::ofstream FurnaceHeaderFile;
            std::string FullPathFurnaceHeaderFile = dir;
            FullPathFurnaceHeaderFile += "/FurnaceHeader";
            FurnaceHeaderFile.open(FullPathFurnaceHeaderFile.c_str(), std::ios::out);

            FurnaceHeaderFile << "Checkpoint version: " << current_version << std::endl;
            FurnaceHeaderFile.close();
...
void
Furnace::writeJobInfo (const std::string& dir, const Real io_time)
{
  ...
  jobInfoFile << " Furnace Job Information\n";
  jobInfoFile << PrettyLine;
  ...
  if (strlen(githash1) > 0) {
    jobInfoFile << "Furnace       git describe: " << githash1 << "\n";
  }
...
#include <furnace_job_info_tests.H>
#ifdef AMREX_PARTICLES
...
Furnace::writeBuildInfo ()
{
  ...
  // build information
  std::cout << PrettyLine;
  std::cout << " Furnace Build Information\n";
  std::cout << PrettyLine;
  ...
  const char* githash3 = buildInfoGetGitHash(3);
  if (strlen(githash1) > 0) {
    std::cout << "Furnace       git describe: " << githash1 << "\n";
  }
  if (strlen(githash2) > 0) {
...
void
Furnace::writePlotFile(const std::string& dir,
                      ostream& os,
                      VisMF::How how)
{
...
void
Furnace::writeSmallPlotFile (const std::string& dir,
                            ostream&       os,
                            VisMF::How     how)
{
...
void
Furnace::plotFileOutput(const std::string& dir,
                       ostream& os,
                       VisMF::How how,
                       const int is_small)
{
  ...
  ParticlePlotFile(dir);
    ...
    if (store_burn_weights) {
        n_data_items += Furnace::burn_weight_names.size();
    }
#endif
#endif
...
#ifndef TRUE_SDC
        if (store_burn_weights) {
            for (auto name: Furnace::burn_weight_names) {
                os << name << '\n';
            }
        }
#endif
...
#ifndef TRUE_SDC
    if (store_burn_weights) {
        MultiFab::Copy(plotMF, getLevel(level).burn_weights, 0, cnt, Furnace::burn_weight_names.size(),0);
        cnt += Furnace::burn_weight_names.size();
    }
#endif
#endif
