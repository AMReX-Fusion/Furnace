...
Furnace::variableSetUp ()
{
  ...
  // Furnace::variableSetUp is called in the constructor of Amr.cpp, so
  ...
  if (ParallelDescriptor::IOProcessor()) {
    ...
    if (strlen(furnace_hash) > 0) {
      std::cout << "\n" << "Furnace git describe: " << furnace_hash << "\n";
    }
  ...
  eos_init(furnace::small_temp, furnace::small_dens);

#ifdef RADIATION
  opacity_init();
#endif

  // Ensure that Furnace's small variables are consistent
  ...
  furnace::small_pres = amrex::max(furnace::small_pres, eos_state.p);
  furnace::small_ener = amrex::max(furnace::small_ener, eos_state.e);
  ...
  ambient::ambient_state[URHO]  = amrex::max(furnace::ambient_density, furnace::small_dens);
  ambient::ambient_state[UTEMP] = amrex::max(furnace::ambient_temp, furnace::small_temp);
  ambient::ambient_state[UEINT] = ambient::ambient_state[URHO] * amrex::max(furnace::ambient_energy,
                                                                            furnace::small_ener);
  ...
  if (UMX != URHO+1) {
    amrex::Error("We are assuming UMX = URHO + 1 in Furnace_setup.cpp");
  }
