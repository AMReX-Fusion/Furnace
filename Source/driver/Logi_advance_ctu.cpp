                       ...
                       int  amr_ncycle)
{
    ...
    BL_PROFILE("Logi::do_advance_ctu()");
...
bool
Logi::retry_advance_ctu(Real dt, advance_status status)
{
    BL_PROFILE("Logi::retry_advance_ctu()");
...
Real
Logi::subcycle_advance_ctu(const Real time, const Real dt, int amr_iteration, int amr_ncycle)
{
    BL_PROFILE("Logi::subcycle_advance_ctu()");
    ...
    while (subcycle_time < (1.0 - eps) * (time + dt)) {
        ...
        if (dt_subcycle <= dt_cutoff * time) {
            if (ParallelDescriptor::IOProcessor()) {
                ...
                std::cout << "  The subcycle mechanism requested subcycled timesteps of maximum length dt = " << dt_subcycle << "," << std::endl
                          ...
                          << "  logi.dt_cutoff, multiplied by the current time (" << dt_cutoff * time << "). Aborting." << std::endl;
            }
        ...
        if (num_subcycles_remaining > max_subcycles) {
            amrex::Print() << std::endl
                           ...
                           << "  If you would like to override this, increase the parameter logi.max_subcycles." << std::endl;
        ...
        if (cfl_violation && !use_retry) {
          amrex::Abort("CFL is too high at this level; go back to a checkpoint and restart with lower CFL number, or set logi.use_retry = 1");
        }
