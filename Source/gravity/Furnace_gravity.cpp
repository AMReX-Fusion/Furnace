...
void
Furnace::construct_old_gravity(int amr_iteration, int amr_ncycle, Real time)
{
    BL_PROFILE("Furnace::construct_old_gravity()");
    ...
    if (gravity->get_gravity_type() == "PoissonGrav" && parent->finestLevel() > 0)
    {
        ...
        if (furnace::verbose && ParallelDescriptor::IOProcessor()) {
...
void
Furnace::construct_new_gravity(int amr_iteration, int amr_ncycle, Real time)
{
    BL_PROFILE("Furnace::construct_new_gravity()");
...
void Furnace::construct_old_gravity_source(MultiFab& source, MultiFab& state_in, Real time, Real dt)
{
    BL_PROFILE("Furnace::construct_old_gravity_source()");
    ...
    AMREX_ALWAYS_ASSERT(furnace::grav_source_type >= 1 && furnace::grav_source_type <= 4);
...
#pragma omp parallel
    ...
    for (MFIter mfi(state_in, TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        [=] AMREX_GPU_HOST_DEVICE (int i, int j, int k)
        {
            ...
            if (furnace::grav_source_type == 1 || furnace::grav_source_type == 2) {
    ...
    if (furnace::verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Furnace::construct_old_gravity_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
void Furnace::construct_new_gravity_source(MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt)
{
    BL_PROFILE("Furnace::construct_new_gravity_source()");
...
#pragma omp parallel
    ...
    if (furnace::verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Furnace::construct_new_gravity_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
