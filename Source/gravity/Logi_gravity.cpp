...
void
Logi::construct_old_gravity(int amr_iteration, int amr_ncycle, Real time)
{
    BL_PROFILE("Logi::construct_old_gravity()");
    ...
    if (gravity->get_gravity_type() == "PoissonGrav" && parent->finestLevel() > 0)
    {
        ...
        if (logi::verbose && ParallelDescriptor::IOProcessor()) {
...
void
Logi::construct_new_gravity(int amr_iteration, int amr_ncycle, Real time)
{
    BL_PROFILE("Logi::construct_new_gravity()");
...
void Logi::construct_old_gravity_source(MultiFab& source, MultiFab& state_in, Real time, Real dt)
{
    BL_PROFILE("Logi::construct_old_gravity_source()");
    ...
    AMREX_ALWAYS_ASSERT(logi::grav_source_type >= 1 && logi::grav_source_type <= 4);
...
#pragma omp parallel
    ...
    for (MFIter mfi(state_in, TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        [=] AMREX_GPU_HOST_DEVICE (int i, int j, int k)
        {
            ...
            if (logi::grav_source_type == 1 || logi::grav_source_type == 2) {
    ...
    if (logi::verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Logi::construct_old_gravity_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
void Logi::construct_new_gravity_source(MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt)
{
    BL_PROFILE("Logi::construct_new_gravity_source()");
...
#pragma omp parallel
    ...
    if (logi::verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Logi::construct_new_gravity_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
