...
// static initialization, must be called before Logi::variableSetUp
...
Radiation::Radiation(Amr* Parent, Logi* logi, int restart)
  : parent(Parent)
{
  // logi is passed in, rather than obtained from parent, because this
                             ...
                             Real time, int level)
{
  ...
  Logi      *logi = (Logi*)&parent->getLevel(level);
  const BoxArray& grids = logi->boxArray();
  const DistributionMapping& dmap = logi->DistributionMap();
...
void Radiation::filBndry(BndryRegister& bdry, int level, Real time)
{
  ...
  Real old_time = logi->get_state_data(Rad_Type).prevTime();
  Real new_time = logi->get_state_data(Rad_Type).curTime();
  Real eps = (new_time > old_time) ? 0.001*(new_time - old_time) : 1.0;

  BL_ASSERT( (time > old_time-eps) && (time < new_time + eps));

  MultiFab& S_new = logi->get_new_data(Rad_Type);
  // the next line is OK even if S_old is not defined yet
  MultiFab& S_old = logi->get_old_data(Rad_Type);
                              ...
                              int igroup)
{
  ...
  FillPatchIterator fpi(*logi, S_new, 1, time, State_Type, 0, nstate);
  ...
  if (level < parent->maxLevel() &&
    ...
    if (indx == 0) {
      // clean up fine-fine interfaces (does all groups at once)
        sync_flux.ClearInternalBorders(logi->Geom());
    }

    Real scale = delta_t_old[level] / delta_t;
    if (!do_sync) scale = 0.0;

    sync_flux.Reflux(rhs, scale, indx, 0, 1, logi->Geom());
  ...
  for (int flev = level; flev > 0; flev--) {
    ...
    if (flux_cons_old[flev] &&
      ...
      if (fgrids == old_boxes) {
        ...
        ref_sync_flux.Reflux(rhs, scale, 0, 0, 1, logi->Geom());
...
void Radiation::get_rosseland_v_dcf(MultiFab& kappa_r, MultiFab& v, MultiFab& dcf,
                                    Real delta_t, Real c,
                                    AmrLevel* logi, int igroup)
{
    BL_ASSERT(kappa_r.nGrow() == 1);
    BL_ASSERT(      v.nGrow() == 1);
    BL_ASSERT(    dcf.nGrow() == 1);

    int nstate = logi->get_new_data(State_Type).nComp();
    Real time = logi->get_state_data(State_Type).curTime();

    FillPatchIterator fpi_r(*logi, kappa_r, 1, time, Rad_Type, 0, 1);
    MultiFab& Er = fpi_r.get_mf();

    FillPatchIterator fpi_s(*logi, kappa_r, 1, time, State_Type, 0, nstate);
...
void Radiation::filter_prim(int level, MultiFab& State)
{
  ...
  FillPatchIterator fpi(*logi,State,ngrow,time,State_Type,0,ncomp);
