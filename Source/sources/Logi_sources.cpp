...
void
Logi::apply_source_to_state(MultiFab& target_state, MultiFab& source, Real dt, int ng)
{
    BL_PROFILE("Logi::apply_source_to_state()");
...
void
Logi::time_center_source_terms(MultiFab& S_new, MultiFab& src_old, MultiFab &src_new, Real dt)
{
  BL_PROFILE("Logi::time_center_source_terms()");
                ...
                MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt, bool apply_to_state)
{
    ...
    BL_PROFILE("Logi::do_old_sources()");
    ...
    if (verbose > 0)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
          std::cout << "Logi::do_old_sources() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
                ...
                MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt, bool apply_to_state)
{
    ...
    BL_PROFILE("Logi::do_new_sources()");
    ...
    if (verbose > 0)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
          std::cout << "Logi::do_new_sources() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
void
Logi::construct_old_source(int src, MultiFab& source, MultiFab& state_in, Real time, Real dt)
{
    BL_PROFILE("Logi::construct_old_source()");
...
void
Logi::construct_new_source(int src, MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt)
{
    BL_PROFILE("Logi::construct_new_source()");
...
Vector<Real>
Logi::evaluate_source_change(const MultiFab& source, Real dt, bool local)
{

  BL_PROFILE("Logi::evaluate_source_change()");
...
void
Logi::print_source_change(Vector<Real> update)
{
...
void
Logi::evaluate_and_print_source_change (const MultiFab& source, Real dt, std::string source_name)
{
    bool local = true;
...
void
Logi::print_all_source_changes(Real dt, bool is_new)
{
...
void
Logi::make_sdc_hydro_plus_sources(MultiFab& source, Real dt)
{
  BL_PROFILE("Logi::make_sdc_hydro_plus_sources()");
...
#ifdef REACTIONS
void
Logi::get_react_source_prim(MultiFab& react_src, Real time, Real dt)
{

    BL_PROFILE("Logi::get_react_source_prim()");
