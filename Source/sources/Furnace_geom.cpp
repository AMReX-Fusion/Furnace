...
///
void
Furnace::construct_old_geom_source(MultiFab& source, MultiFab& state_in, Real time, Real dt)
{
  ...
  if (verbose > 1)
  {
      ...
      if (ParallelDescriptor::IOProcessor())
          std::cout << "Furnace::construct_old_geom_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
      });
#endif
    }
...
void
Furnace::construct_new_geom_source(MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt)
{
  ...
  if (verbose > 1)
  {
      ...
      if (ParallelDescriptor::IOProcessor())
          std::cout << "Furnace::construct_new_geom_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
      });
#endif
    }
...
void
Furnace::fill_geom_source (Real time, Real dt,
