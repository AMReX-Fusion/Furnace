...
void
Furnace::construct_old_sponge_source(MultiFab& source, MultiFab& state_in, Real time, Real dt)
{
...
void
Furnace::construct_new_sponge_source(MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt)
{
...
#pragma omp parallel
    ...
    if (verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor()) {
            std::cout << "Furnace::construct_new_sponge_source() time = " << run_time << "\n" << "\n";
        }
#ifdef BL_LAZY
        });
#endif
...
void
Furnace::apply_sponge(const Box& bx,
