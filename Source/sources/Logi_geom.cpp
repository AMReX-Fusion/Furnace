...
///
void
Logi::construct_old_geom_source(MultiFab& source, MultiFab& state_in, Real time, Real dt)
{
  ...
  if (verbose > 1)
  {
      ...
      if (ParallelDescriptor::IOProcessor())
          std::cout << "Logi::construct_old_geom_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
      });
#endif
    }
...
void
Logi::construct_new_geom_source(MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt)
{
  ...
  if (verbose > 1)
  {
      ...
      if (ParallelDescriptor::IOProcessor())
          std::cout << "Logi::construct_new_geom_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
      });
#endif
    }
...
void
Logi::fill_geom_source (Real time, Real dt,
