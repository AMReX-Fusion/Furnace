...
void
Logi::construct_old_sponge_source(MultiFab& source, MultiFab& state_in, Real time, Real dt)
{
...
void
Logi::construct_new_sponge_source(MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt)
{
...
#pragma omp parallel
    ...
    if (verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor()) {
            std::cout << "Logi::construct_new_sponge_source() time = " << run_time << "\n" << "\n";
        }
#ifdef BL_LAZY
        });
#endif
...
void
Logi::apply_sponge(const Box& bx,
