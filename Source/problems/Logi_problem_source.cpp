...
void
Logi::construct_old_ext_source(MultiFab& source, MultiFab& state_in, Real time, Real dt)
{
    ...
    if (verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Logi::construct_old_ext_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
void
Logi::construct_new_ext_source(MultiFab& source, MultiFab& state_old, MultiFab& state_new, Real time, Real dt)
{
    const Real strt_time = ParallelDescriptor::second();
    ...
    if (verbose > 1)
    {
        ...
        if (ParallelDescriptor::IOProcessor())
            std::cout << "Logi::construct_new_ext_source() time = " << run_time << "\n" << "\n";
#ifdef BL_LAZY
        });
#endif
    }
...
void
Logi::fill_ext_source (const Real time, const Real dt, const MultiFab& state_old, const MultiFab& state_new, MultiFab& ext_src)
{
