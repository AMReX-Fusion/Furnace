...
Logi implements a constrained transport (CT) corner transport upwind
...
.. note::
   ...
   If you wish to express the magnetic field in Gauss, then you will need to multiply the value Logi carries
   by :math:`\sqrt{4\pi}`.
...
controlled by ``logi.ppm_type``, with ``0`` selecting piecewise
