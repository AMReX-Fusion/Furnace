...
The hydrodynamics scheme in Logi implements an unsplit
...
Logi uses the primitive form of the fluid equations, defined in terms of
...
parameters logi.dual_energy_eta1, logi.dual_energy_eta2, and
logi.dual_energy_eta3. We then consider the ratio :math:`e_T / E`,
...
.. index:: logi.do_hydro, logi.add_ext_src, logi.do_sponge, logi.normalize_species, logi.spherical_star, logi.show_center_of_mass
...
-  ``logi.do_hydro``: time-advance the fluid dynamical
   equations (0 or 1; must be set)

-  ``logi.add_ext_src``: include additional user-specified
   source term (0 or 1; default 0)

-  ``logi.do_sponge``: call the sponge routine
   after the solution update (0 or 1; default: 0)

   See :ref:`sponge_section` for more details on the sponge.

-  ``logi.normalize_species``: enforce that :math:`\sum_i X_i = 1`
   (0 or 1; default: 0)

-  ``logi.spherical_star``: this is used to set the boundary
   conditions by assuming the star is spherically symmetric in
   the outer regions (0 or 1; default: 0)

   When used, Logi averages the values at a given radius over the
...
-  ``logi.show_center_of_mass``: (0 or 1; default: 0)

.. index:: logi.small_dens, logi.small_temp, logi.small_pres

Several floors are imposed on the thermodynamic quantities to prevet unphysical
behavior:

-  ``logi.small_dens``: (Real; default: -1.e20)

-  ``logi.small_temp``: (Real; default: -1.e20)

-  ``logi.small_pres``: (Real; default: -1.e20)
...
-  logi.use_flattening turns on/off the flattening of parabola
...
limiters is determined by logi.ppm_type.
...
-  ``logi.ppm_type`` : use piecewise linear vs PPM algorithm (0 or 1;
   default: 1).  A value of 1 is the standard piecewise parabolic
   reconstruction.

-  ``logi.ppm_temp_fix`` does various attempts to use the
...
-  ``logi.transverse_use_eos`` : If this is 1, then we call
...
-  ``logi.transverse_reset_density`` : If the transverse
...
-  ``logi.transverse_reset_rhoe`` : The transverse updates operate
...
Logi has three main options for the Riemann solver—the
...
.. index:: logi.riemann_speed_limit
...
.. note::
   ...
   kicking in.  This behavior can be changed with the ``logi.riemann_speed_limit``
   parameter.
...
-  ``logi.riemann_solver``: this can be one of the following values:
   ...
   as described in the original Logi paper :cite:`logi_I`.

   The Colella & Glaz solver is iterative, and two runtime parameters are used
   to control its behavior:

   -  ``logi.cg_maxiter`` : number of iterations for CG algorithm
      (Integer; default: 12)

   -  ``logi.cg_tol`` : tolerance for CG solver when solving
      for the “star” state (Real; default: 1.0e-5)

   -  ``logi.cg_blend`` : this controls what happens if the root
...
-  ``logi.hybrid_riemann`` : switch to an HLL Riemann solver when we are
...
prediction. The options are controlled by ``logi.ppm_temp_fix``,
which takes values:
...
occur as a result of a hydro update. Logi contains a prescription
...
logi.limit_fluxes_on_small_dens.
...
Logi implements the hybrid momentum scheme of :cite:`byerly:2014`.
