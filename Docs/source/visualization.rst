...
There are a large number of tools that can be used to read in Furnace
or AMReX data and make plots.  These tools all work from Furnace
...
results such as those Furnace produces. As yt is script-based, it’s not
...
Using the plotfiles generated in the example in the :doc:`getting_started` section, here we demonstrate how to use ``yt`` to load and visualize data. This section was generated from a Jupyter notebook which can be found in ``Docs/source/yt_example.ipynb`` in the Furnace repo. 
