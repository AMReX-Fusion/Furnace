...
Logi is written in a modular fashion so that the EOS and network
...
Logi relies on routines to calculate the equation of state (EOS)
...
Most of the standard problem setups in Logi (such as the Sedov blast wave)
...
``logi.T_guess``, and should be set to a sensible value for each problem
...
rest of Logi. At the beginning of the simulation, ``eos_init``
...
Logi operation. The main quantities that any EOS in any mode needs to
...
components through a nuclear burning step. Logi comes with a ``general_null``
...
In normal operation in Logi  the integration occurs over a time interval
...
reactions to occur in a zone using the parameters ``logi.react_T_min``,
``logi.react_T_max``, ``logi.react_rho_min`` and ``logi.react_rho_max``.
