...
Logi-specific ones are interpreted by ``Make.Logi``.

A lot of optional features are enabled at compile time.  This allows
Logi to reduce the memory footprint of the state arrays by not allocating
space for variables that are not used.

General Build Parameters
^^^^^^^^^^^^^^^^^^^^^^^^

.. index:: USE_ALL_LOGI, USE_AMR_CORE, USE_HYPRE

These Parameters affect the build (parallelism, performance, etc.)
Most of these are parameters from AMReX.

  * ``USE_ALL_LOGI``: compile all of the core Logi directories.
    ...
    do not need all of the Logi directories compiled.  

  * ``USE_AMR_CORE``: compile all of the core AMReX directories, including
    ``Base/``, ``AmrCore/``, ``Amr/``, and ``Boundary/``.  This defaults
    to ``TRUE`` and should be left set for Logi simulations.  The purpose
...
are MHD, radiation, and anything using a pynulogi-generated network.
These parameters control Fortran support:
  ...
  * ``USE_FORT_MICROPHYSICS``: if set to ``TRUE``, then Fortran
    ...
    are not using a pynulogi network, then you can probably set this
    to ``FALSE``.
...
is handled.
  ...
  * ``USE_TRUE_SDC``: use the true SDC method to couple hydro and
    ...
    with ``logi.time_integration_method = 2``.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  * ``USE_HYBRID_MOMENTUM``: have Logi evolve angular momentum in addition to linear
    momentum.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  * ``USE_AUX_UPDATE``: some networks define auxillary quantities, which in general
    Logi will advect, but not otherwise change.  If we set ``USE_AUX_UPDATE=TRUE``
    then Logi will call a user-supplied routine ``advance_aux()`` that can
    change the auxillary quantities.

    .. index:: USE_AUX_UPDATE

  * ``USE_POST_SIM``: if this is defined, then Logi will call the user-defined 
    routine ``problem_post_simulation()`` after the full evolution of the problem
    has ended.

    .. index:: USE_POST_SIM

  * ``USE_MAESTRO_INIT``: this enables the code to allow Logi to restart from a 
...
.. note::
   ...
   ``tmp_build_dir/logi_sources/Nd.COMP.OPTIONS.EXE/``, where ``N`` is the 
...
* ``set_variables.py`` is called
  ...
  * This processes the Logi ``_variables`` file and writes
    ``state_indices.H`` (and  ``state_indices_nd.F90`` if Fortran is enabled) into the
    ``tmp_build_dir/logi_sources/`` directory.
...
* (for ``general_null networks``), ``network_properties.H`` (and
  ...
  * The hook for this is in ``$(LOGI_HOME)/Microphysics/networks/general_null/Make.package``
...
* Runtime parameter files for the microphysics routines are parsed by ``write_probin.py``
  ...
  * This writes the routines that manage the Microphysics runtime
    ...
    ``tmp_build_dir/logi_sources/``.

  * The hook for this is in ``Make.auto_source`` in the rule for ``extern_parameters.H``

* Logi's runtime parameters are parsed by ``parse_logi_params.py``

  .. index:: parse_logi_params.py

  * This writes the C++ header files that manage and read the runtime
    parameters.  These file are output in
    ``tmp_build_dir/logi_sources/``.

  * The hook for this is in ``Make.auto_source`` in the rule for ``logi_params.H``
...
* Problem-specific runtime parameters are parsed by ``write_probdata.py``
  ...
  * If the problem directory defines a ``_prob_params`` then it is parsed
    ...
    variables in ``Logi/Source/problems/_default_prob_params`` will be included.

  * The script ``Logi/Util/scripts/write_probdata.py`` is used

  * The hook for this is in ``Make.auto_source`` in the ``prob_parameters.H`` rule.

  * These headers are output into ``tmp_build_dir/logi_sources/``.
